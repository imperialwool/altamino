...
async def change_aminoId(request: Request):
    ...
    if not DataValidator.check_timestamp(data.get("timestamp", 0)):
        return Errors.InvalidRequest(timestamp()-t1)
...
async def get_user_wall(uid, request: Request, start: int = 0, size: int = 25, sort: str = "newest"):
    ...
    trigger_uid = DataValidator.from_sid_to_uid(request)
...
async def delete_post_from_wall(request: Request, uid: str, commentId: str):
    t1 = timestamp()
    if not await DataValidator.is_request_valid(request, need_auth=True):
        return Errors.InvalidRequest(timestamp()-t1)
...
async def ban_user(uid, request: Request):
    ...
    target_user = DataValidator.from_sid_to_uid(request)
...
async def follow_user(uid, request: Request):
    ...
    suid = DataValidator.from_sid_to_uid(request)
...
async def get_wallet_info(request: Request):
    ...
    db = await Database().init()
    table = await db.get(table="Users")
    row = await table.find_one({"id": DataValidator.from_sid_to_uid(request)})
    if row == None:
        return Errors.AccountNotExist(timestamp()-t1)
    await db.close()
    return Base.Answer({
...
async def edit_user_info(uid, request: Request):
    data = await request.json()
    checkSid, t1 = DataValidator.check_sid(request.headers['NDCAUTH']), timestamp()
    if not checkSid[0]:  return checkSid[1]
    if uid != checkSid[1]: return Errors.InvalidRequest(timestamp()-t1)
    if not await DataValidator.is_request_valid(request, is_post=True, need_auth=True):
        return Errors.InvalidRequest(timestamp()-t1)
