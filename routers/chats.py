...
async def edit_chat(chatId: str, request: Request):
    t1 = timestamp()
    if not await DataValidator.is_request_valid(request, need_auth=True, is_post=True):
        return Errors.InvalidRequest(timestamp()-t1)
    
    try:
        data = await request.json()

        if not DataValidator.check_timestamp(data["timestamp"]):
            raise Exception()
    except:
        return Errors.InvalidRequest(timestamp()-t1)
...
async def mark_as_read(request: Request, chatId: str):
    t1 = timestamp()
    if not await DataValidator.is_request_valid(request, False, True):
        return Errors.InvalidRequest(timestamp()-t1)
