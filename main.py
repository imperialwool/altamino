...
from routers.blockSuspects import blockSuspects, block_sus_users, is_sus_in_cage

# app things

app = FastAPI(docs_url=None, redoc_url=None)
app.include_router(blockSuspects)
...
async def custom_403_handler(_, __):
    return await is_sus_in_cage(_) or await block_sus_users(_, response=Errors.Forbidden())

@app.exception_handler(404)
@app.exception_handler(405)
async def custom_404_handler(_, __):
    return await is_sus_in_cage(_) or await block_sus_users(_, response=Errors.InvalidPath())
